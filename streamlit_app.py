...
# ================== Poker GTO Helper — Unified App (embedded preflop ranges) ==================
st.set_page_config(page_title="Poker GTO Helper", page_icon="♠️", layout="wide")
st.title("♠️ Poker GTO Decision Helper — Unified")
st.caption("Preflop uses embedded GTO-style ranges (169×6 heuristic). Postflop uses GTO-inspired model. "
           "Plotly table, auto-call allocation, action mix display. One-file deployable.")

# -------- Core card/rank utils --------
...
# ----- Build/Embed heuristic GTO-style preflop ranges (169×6) -----
BASE = {"A":10,"K":8,"Q":7,"J":6,"T":5,"9":4.5,"8":4,"7":3.5,"6":3,"5":2.5,"4":2,"3":1.5,"2":1}
TIGHTNESS = {"UTG":1.00,"MP":0.92,"CO":0.84,"BTN":0.75,"SB":0.85,"BB":0.95}

def chen_score_combo(hand: str) -> float:
    order = "23456789TJQKA"
    if len(hand)==2:
        r = hand[0]
        return max(5.0, BASE[r]*2)
    hi, lo, suited = hand[0], hand[1], (hand[2]=="s")
    base = BASE[hi]
    score = base + (2 if suited else 0)
    gap = order.index(hi) - order.index(lo) - 1
    if gap==1: score -= 1
    elif gap==2: score -= 2
    elif gap==3: score -= 4
    elif gap>=4: score -= 5
    if order.index(hi) <= order.index("Q") and gap==0: score += 1
...
def all_169():
    ...
    combos = []
    # pairs
    for r in order[::-1]: combos.append(r+r)
    # non-pairs hi>lo, s/o
    for hi in order[::-1]:
        for lo in order[::-1]:
            if order.index(hi) <= order.index(lo): continue
            for s in ("s","o"): combos.append(f"{hi}{lo}{s}")
    return combos

def open_freq(score: float, position: str) -> float:
    base_t = 12.0 * TIGHTNESS[position]
    z = (score - base_t) / 2.2
    return _clamp(1/(1+math.exp(-z)))

def build_gto_ranges():
    G = {p:{} for p in ["UTG","MP","CO","BTN","SB","BB"]}
    for pos in G:
        for c in all_169():
            s = chen_score_combo(c)
            p_open = open_freq(s, pos)
            # split into small/big sizing
            small_share = 0.65
            if pos in ("CO","BTN"): small_share = 0.55
            if len(c)==3 and c[2]=="s" and c[0] in "AKQJT":
                small_share -= 0.07
            mix = {
                "open_small": p_open * small_share,
                "open_big":   p_open * (1-small_share),
                "fold":       1 - p_open
            }
            G[pos][c] = mix
    return G
...
@st.cache_resource
def get_gto_ranges():
    return build_gto_ranges()

GTO_RANGES = get_gto_ranges()
...
def preflop_from_chart(hand: str, position: Position) -> Dict[str, float]:
    combo, hi, lo, suited = normalize_combo(hand)
    chart = GTO_RANGES.get(position, {})
    mix = chart.get(combo, {"open_small":0.0,"open_big":0.0,"fold":1.0})
    # Aggregate to buckets (no CALL in unopened pot; SB limps are not modeled here)
    agg = {"CALL":0.0, "RAISE": (mix["open_small"] + mix["open_big"]), "CHECK":0.0, "FOLD": mix["fold"]}
    total = sum(agg.values()) or 1.0
    for k in agg: agg[k] = agg[k]/total
    return agg

def gto_preflop_decide(hand: str, position: Position, unopened: bool,
                       facing_bet: float, stack_bb: float,
                       players_left:int, total_players:int) -> Dict[str, float]:
    """Enforce chart for unopened. For facing open, use a simple 3-bet/call heuristic."""
    if unopened:
        agg = preflop_from_chart(hand, position)
        return agg
    # Facing an open: derive from Chen score vs thresholds (still heuristic)
    combo, hi, lo, suited = normalize_combo(hand)
    s = chen_score_combo(combo if len(combo)!=2 else combo)  # same function works
    # thresholds: value 3bet ~ premium; call with medium; fold weakest
    t_val = 15.0 - (stack_bb-20)/40
    t_call = 10.0
    p_raise = _clamp((s - t_val)/6.0)
    p_call  = _clamp((s - t_call)/5.0) * (1.0 - p_raise*0.6)
    p_fold  = _clamp(1.0 - _clamp(p_raise + p_call))
    tot = p_raise + p_call + p_fold or 1.0
    return {"RAISE": p_raise/tot, "CALL": p_call/tot, "CHECK": 0.0, "FOLD": p_fold/tot}
...
def gto_postflop_mix(ctx: 'PostflopContext') -> Dict[str,str]:
    ...
    if ctx.facing_bet_bb>0:
        ...
        allp = {"CALL": p_call, "RAISE": p_raise, "CHECK": 0.0, "FOLD": p_fold}
        tot = sum(allp.values()) or 1.0
        for k in allp: allp[k] = allp[k]/tot
        best, best_p = max(allp.items(), key=lambda kv: kv[1])
        return {"primary":best,"best":best,"best_pct":best_p,"mix":" / ".join([f"{k} {_pct_round(v)}%" for k,v in allp.items()]),"all": allp,
                "explain":f"{ctx.street.upper()} vs {bet:.1f} into {pot:.1f} | N={ctx.n_players} | hs~{hs*100:.0f}% | adv~{adv:+.2f} | SPR~{spr:.1f}"}
    ...
    agg = {"CALL":0.0,"RAISE":sum(p for lbl,p in mix_parts if lbl.startswith("BET")),"CHECK":p_check,"FOLD":0.0}
    tot = sum(agg.values()) or 1.0
    for k in agg: agg[k] = agg[k]/tot
    best, best_p = max(agg.items(), key=lambda kv: kv[1])
    return {"primary":primary,"best":best,"best_pct":best_p,"mix":" / ".join([f\"{lbl} {_pct_round(p)}%\" for lbl,p in mix_parts]),"all": agg,
            "explain":f"{ctx.street.upper()} | N={ctx.n_players} | hs~{hs*100:.0f}% | adv~{adv:+.2f} | SPR~{spr:.1f}"}
...
def plotly_table(seat_count:int, btn_seat:int, hero_seat:int, pos_by_seat:Dict[int,str],
    ...
    oval = dict(type="circle", xref="x", yref="y", x0=10, y0=10, x1=90, y1=90, line=dict(color="#1f2937", width=6), fillcolor="#0f172a")
    fig.add_shape(oval)
    ...
    for i in range(seat_count):
        ...
        if put>0: fig.add_annotation(x=x, y=y-6.5, text=f"<b>{put:.2f}</b>", showarrow=False, font=dict(color="#ffffff", size=11), bgcolor="#ef4444")
    if len(hero_codes)==2:
        i = hero_seat
        angle = -_m.pi/2 + (2*_m.pi * i / seat_count)
...
# Personalities -> exploit (used postflop only here)
weights=[]
for i in range(SEATS):
...
with col_left:
    ...
    st.markdown(f"**Players in Pot:** {players_in_pot} • **Exploit (postflop):** {exploit_adv:+.2f}")
with col_right:
...
if errs:
    ...
    if street=="preflop":
        # Force preflop to use chart for unopened pots.
        position = hero_pos_label if hero_pos_label in ["UTG","MP","CO","BTN","SB","BB"] else "MP"
        unopened = (current_bet_now <= 1.0)  # blinds only -> unopened to hero
        agg = gto_preflop_decide(
            hand=hero_hand, position=position, unopened=unopened,
            facing_bet=current_bet_now, stack_bb=float(stack_bb),
            players_left=int(players_left), total_players=int(total_players)
        )
        best, best_p = max(agg.items(), key=lambda kv: kv[1])
        result = {"best": best, "best_pct": best_p, "all": agg}
    else:
    ...
    if result:
        ...
        st.caption("Preflop uses chart if unopened; facing raises uses heuristic 3-bet/call. Postflop is GTO-inspired.")
...
st.caption("This single-file app is ready for Streamlit Cloud. Preflop chart is embedded; no external files required.")
